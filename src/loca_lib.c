...
static char *cvs_conlib_id =
  "$Id: loca_lib.c,v 1.9 2009/07/28 00:10:04 ljfrink Exp $";
#endif
...
static double solution_scale(struct con_struct *con,
                             struct arc_scale_struct *arc);
double scaled_dot_prod(double *x, double *y, double *sc, int n);
static void print_cont_step1(int order, double step,
                             double step_old, struct con_struct *con);
static void print_cont_step2(int order, double step, struct con_struct *con);
static void print_cont_step_fail(int order, double step,
...

{
  ...
  int     sn_old, sn_new;                /* Sign of cpi->dp_ds, to check for a
                                        turning point                        */
  ...
  double *x_tang_old, *x2_old, *y_vec_old, *z_vec_old, bif_param_old;
  ...
  double omega_old; /* old Hopf tracking frequency                          */
  ...
  /* Fork to MF linrary if doing multiparameter continuation */

#ifdef LOCA_MF
  if (cgi->method == MANIFOLD_CONTINUATION) {
    loca_mf_driver(con);
    return 1;
  }
#endif
...
/*printf("cpi:nstep=%d\n",cpi->nstep);*/
  ...
  for (n = 0; n <= csi->max_steps; n++) {
    ...
    if (num_newt_conv < 0) {
      ...
      if (cpi->step_num != 0 && end_passed <= 0.0) csi->last_step = TRUE;
...
double solution_scale(struct con_struct *con, struct arc_scale_struct *arc)
  ...
  umag = arc->dx0 * arc->dx0 * scaled_dot_prod
         (cpi->x_tang, cpi->x_tang, cpi->scale_vec, cgi->numOwnedUnks);

  /* Adjust cpi->scale_vec by current scale factor arc->dx_fac. */

  for (i = 0; i < cgi->numUnks; i++) cpi->scale_vec[i] *= arc->dx_fac;

  arc->umag2 = scaled_dot_prod (cpi->x_tang, cpi->x_tang,
                                cpi->scale_vec, cgi->numOwnedUnks);
  ...
  if (cpi->step_num == 0 && cai->dp_ds2_goal > 1e-6) {
    arc->dx_fac = cpi->dp_ds * sqrt( (1.0 - cai->dp_ds2_goal) /
                     (1.0 - cpi->dp_ds * cpi->dp_ds) ) / arc->dp_ds_goal;
    arc->dx0 = arc->dx_fac;
    for (i = 0; i < cgi->numUnks; i++) cpi->scale_vec[i] *= arc->dx_fac;
    arc->umag2 = scaled_dot_prod(cpi->x_tang, cpi->x_tang,
                               cpi->scale_vec, cgi->numOwnedUnks);
  ...
  /* Limit arc->dx_fac to arc->dx_fac_max to avoid division by zero. */
    ...
    if(cpi->dp_ds > arc->dp_ds_limit) arc->dx_fac = arc->dx_fac_max;

    else arc->dx_fac *= cpi->dp_ds * 
          sqrt( (1.0 - cai->dp_ds2_goal) /
           (1.0 - cpi->dp_ds * cpi->dp_ds) )
           / arc->dp_ds_goal;
...
/*****************************************************************************/
/*****************************************************************************/
static void print_cont_step1(int order, double step, double step_old,
                             struct con_struct *con)
...

{
  ...
  if (cgi->method == TURNING_POINT_CONTINUATION)
      printf("\tTurning Point located at:  %10g  %10g\n",
                cgi->param, con->turning_point_info.bif_param);
  else if (cgi->method == PHASE_TRANSITION_CONTINUATION)
      printf("\tPhase Transition located at:  %10g  %10g\n",
                cgi->param, con->phase_transition_info.bif_param);
  else if (cgi->method == PITCHFORK_CONTINUATION)
      printf("\tPitchfork Bifurcation located at:  %10g  %10g\n",
                cgi->param, con->pitchfork_info.bif_param);
  else if (cgi->method == HOPF_CONTINUATION)
      printf("\tHopf Bifurcation located at:  %10g  %10g  %10g\n",
                cgi->param, con->hopf_info.bif_param, con->hopf_info.omega);
  else printf("\tOrder   = %d\n", order);

  if (order == 2) printf("\t delta_s = %g", cpi->arc_step);
  printf("\tdelta_c_p = %g\n", step);
...
{
  int i;
  for (i = 0; i < ntimes-1; i++) printf("%c", *charstr);
  printf("\n");
}
