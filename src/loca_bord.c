...
static char *cvs_contbord_id =
  "$Id: loca_bord.c,v 1.8 2007/03/09 20:57:47 ljfrink Exp $";
#endif
*/
...

{
 ...
 /*************************** BEGIN EXECUTION *******************************/
  ...
  calc_rhs_continuation(ARC_CONT_SOL2, x, dx2, NULL, NULL, NULL, cgi->param,
  ...
  al_eq_soln_contrib =
      scaled_dot_prod(x, cpi->x_tang, cpi->scale_vec, cgi->numOwnedUnks)
  ...
  y = (arc_length_equation - 
     ...
     / (cpi->dp_ds - 
       scaled_dot_prod(cpi->x_tang, dx2, cpi->scale_vec, cgi->numOwnedUnks));
  ...
  for (i=0; i < cgi->numUnks; i++) delta_x[i] -= y * dx2[i];
  ...
  scaled_resid = fabs(arc_length_equation)
                         / (reltol * fabs(cpi->arc_step) + abstol);
  ...
  if (cgi->printproc > 4) {
	...
	scaled_resid, arc_length_equation, cpi->arc_step+arc_length_equation);
    printf("\t***********************************************************\n");
  }
...
AGS_option=0;
 ...
 /*************************** BEGIN EXECUTION *******************************/
  ...
  for (i=0; i < cgi->numUnks; i++) a[i] = -delta_x[i];
  ...
  calc_rhs_continuation(TP_CONT_SOL2, x, b, NULL, NULL, NULL, 
		        con->turning_point_info.bif_param, cgi->perturb,
...
*/
    ...
    if (cgi->nv_restart == FALSE) {
      ...
      for (i=0; i < cgi->numUnks; i++) y[i] = b[i]/vecnorm;
  ...
  /* Load in old null vector if specified */
      ...
      if (cgi->printproc > 7) printf("Loading in previous null vector ...\n");

      for (i=0; i < cgi->numUnks; i++) y[i] = con->turning_point_info.nv[i];

      calc_scale_vec_conwrap(x, phi, cgi->numUnks);

      vecnorm = ltransnorm(y, phi);

      for (i=0; i < cgi->numUnks; i++) y[i] /= vecnorm;
...
*/
  ...
  for (i=0; i < cgi->numUnks; i++) a[i] += a_big*b[i]; /*SCALED*/
  for (i=0; i < cgi->numUnks; i++) b[i] *= b_big; /*SCALED*/
#endif

  /* Next, "c" is calculated as a function of a and y. */

  calc_rhs_continuation(TP_CONT_SOL3, x, c, a, phi, x_tmp,
		        con->turning_point_info.bif_param, cgi->perturb,
                        y, cgi->numUnks, cgi->numOwnedUnks);
  ...
  calc_rhs_continuation(TP_CONT_SOL4, x, d, b, phi, x_tmp, 
		        con->turning_point_info.bif_param, cgi->perturb,
                        y, cgi->numUnks, cgi->numOwnedUnks);
  ...
  for (i=0; i < cgi->numUnks; i++) c[i] += c_big*d[i]; /*SCALED*/
  for (i=0; i < cgi->numUnks; i++) d[i] *= d_big; /*SCALED*/
#endif

  dt_p = ( (1.0 - AGS_option*ltransnorm(y, phi)) - ltransnorm(c, phi) )
         / ltransnorm(d, phi);
  ...
  for (i=0; i < cgi->numUnks; i++) delta_x[i] = -a[i];

  for (i=0; i < cgi->numUnks; i++) delta_x[i] -= dt_p * b[i];
...
*/
  ...
  for (i=0; i < cgi->numUnks; i++) y[i] += c[i];
  ...
  if (cgi->printproc > 4) {
    ...
    printf("\t***********************************************************\n");
    printf("\tparameter    %e        %e    %g\n",
           param_update, dt_p, con->turning_point_info.bif_param);
...
AGS_option=1;
 ...
 /*************************** BEGIN EXECUTION *******************************/
  ...
  if (first) {
    ...
    for (i=0; i < cgi->numUnks; i++) psi[i] = psi[i]/tmp;
  ...
  for (i=0; i < cgi->numUnks; i++) phi[i] /= vecnorm;
  ...
  calc_rhs_continuation(TP_CONT_SOL2, x, b, NULL, NULL, NULL,
		        con->pitchfork_info.bif_param, cgi->perturb,
                        NULL, cgi->numUnks, cgi->numOwnedUnks);
  ...
  for (i=0; i < cgi->numUnks; i++) c[i] = -psi[i];

  i = linear_solver_conwrap(c, OLD_JACOBIAN, NULL);

  /* Next, "d" is calculated as a function of a and phi. */

  calc_rhs_continuation(TP_CONT_SOL3, x, d, a, cpi->scale_vec, x_tmp,
		        con->pitchfork_info.bif_param, cgi->perturb,
                        phi, cgi->numUnks, cgi->numOwnedUnks);
  ...
  if (AGS_option==1) for (i=0; i < cgi->numUnks; i++) d[i] += phi[i];

  /* Next, "e" is calculated as a function of b and phi. */

  calc_rhs_continuation(TP_CONT_SOL4, x, e, b, cpi->scale_vec, x_tmp,
		        con->pitchfork_info.bif_param, cgi->perturb,
                        phi, cgi->numUnks, cgi->numOwnedUnks);
  ...
  calc_rhs_continuation(TP_CONT_SOL3, x, f, c, cpi->scale_vec, x_tmp,
		        con->pitchfork_info.bif_param, cgi->perturb,
                        phi, cgi->numUnks, cgi->numOwnedUnks);

  i = linear_solver_conwrap(f, SAME_BUT_UNSCALED_JACOBIAN, x_tmp);

  if (AGS_option==1) for (i=0; i < cgi->numUnks; i++) f[i] += phi[i];
...
*/
  ...
  for (i=0; i < cgi->numUnks; i++) phi[i] += c[i];
  ...
  param_update = fabs(dt_p)  /
                 (reltol * fabs(con->pitchfork_info.bif_param) + abstol);
  ...
  if (cgi->printproc > 4) {
    ...
    printf("\t***********************************************************\n");
    printf("\tparameter    %e        %e    %g\n",
           param_update, dt_p, con->pitchfork_info.bif_param);
...

{
 ...
 /*************************** BEGIN EXECUTION *******************************/
  ...
  if (first) {
	...
	if(cpi->mass_param==1)
	  printf("\tMass Matrix is a Function of Bifurcation Parameter!\n");
       else printf("\tMass Matrix is Independent of Bifurcation Parameter!\n");
      }
    } else {
      if (cgi->printproc > 1) 
  ...
  if (cgi->printproc > 7) {
    ...
    else printf("\tdM/d(param) is NOT included in Komplex solves\n");
  }
  ...
  if (first2) {
    ...
    for(i=0; i< cgi->numOwnedUnks; i++) cpi->scale_vec[i]=chi->y_vec[i];
    ...
    for (i=0; i < cgi->numOwnedUnks; i++) {
      ...
      z_tmp = chi->z_vec[i];
      chi->y_vec[i] = 
	(alpha*y_tmp+beta*z_tmp)/(alpha*alpha+beta*beta);
      chi->z_vec[i] = 
	-(beta*y_tmp-alpha*z_tmp)/(alpha*alpha+beta*beta);
    }   
    first2=FALSE;  
  }
  ...
  calc_rhs_continuation(TP_CONT_SOL2, x, b, NULL, NULL, NULL, chi->bif_param,
		        cgi->perturb, NULL, cgi->numUnks, cgi->numOwnedUnks);
  ...
  calc_rhs_continuation(HP_CONT_SOL3, x, c, chi->z_vec, chi->y_vec, 
			x_tmp, chi->bif_param, cgi->perturb, 
			d, cgi->numUnks, cgi->numOwnedUnks);
  ...
  if (cgi->printproc > 4)
    printf("\n\tHopf Continuation: Calculating *e* and *f* vectors"
	   " (Komplex solve)!\n"); 
  calc_rhs_continuation(TP_CONT_SOL3, x, e, a, cpi->scale_vec, x_tmp,
			chi->bif_param, cgi->perturb, chi->y_vec,
			cgi->numUnks, cgi->numOwnedUnks);
  calc_rhs_continuation(TP_CONT_SOL3, x, f, a, cpi->scale_vec, x_tmp,
			chi->bif_param, cgi->perturb, chi->z_vec,
			cgi->numUnks, cgi->numOwnedUnks);
  ...
  if (cpi->mass_x) {
    calc_rhs_continuation(HP_CONT_DMDX, x, M_1, a, cpi->scale_vec, x_tmp,
			chi->bif_param, cgi->perturb, chi->z_vec,
			cgi->numUnks, cgi->numOwnedUnks);
    calc_rhs_continuation(HP_CONT_DMDX, x, M_2, a, cpi->scale_vec, x_tmp,
			chi->bif_param, cgi->perturb, chi->y_vec,
			cgi->numUnks, cgi->numOwnedUnks);
    for (i=0;i<cgi->numUnks;i++) {
      e[i] += M_1[i]*chi->omega;
  ...
  if (cgi->printproc > 4)
    printf("\n\tHopf Continuation: Calculating *g* and *h* vectors"
	   " (Komplex solve)!\n");
  calc_rhs_continuation(TP_CONT_SOL4, x, g, b, cpi->scale_vec, x_tmp,
			chi->bif_param, cgi->perturb, chi->y_vec,
			cgi->numUnks, cgi->numOwnedUnks);
  calc_rhs_continuation(TP_CONT_SOL4, x, h, b, cpi->scale_vec, x_tmp,
			chi->bif_param, cgi->perturb, chi->z_vec,
			cgi->numUnks, cgi->numOwnedUnks);
  if (cpi->mass_x) {
    calc_rhs_continuation(HP_CONT_DMDX, x, M_1, b, cpi->scale_vec, x_tmp,
			chi->bif_param, cgi->perturb, chi->z_vec,
			cgi->numUnks, cgi->numOwnedUnks);
    calc_rhs_continuation(HP_CONT_DMDX, x, M_2, b, cpi->scale_vec, x_tmp,
			chi->bif_param, cgi->perturb, chi->y_vec,
			cgi->numUnks, cgi->numOwnedUnks);
    for (i=0;i<cgi->numUnks;i++) {
      g[i] += M_1[i]*chi->omega;
  ...
  if (cpi->mass_param) {
    vec_copy(x, M_1);
    vec_copy(x, M_2);
    calc_rhs_continuation(HP_CONT_DMDPARAM, x, M_1, a, cpi->scale_vec, x_tmp, 
			  chi->bif_param, cgi->perturb, chi->z_vec,
			  cgi->numUnks, cgi->numOwnedUnks);
    calc_rhs_continuation(HP_CONT_DMDPARAM, x, M_2, a, cpi->scale_vec, x_tmp, 
			  chi->bif_param, cgi->perturb, chi->y_vec,
			  cgi->numUnks, cgi->numOwnedUnks);
    for (i=0;i<cgi->numUnks;i++) {
      g[i] += M_1[i]*chi->omega;
  ...
  i = komplex_linear_solver_conwrap(g, h, OLD_JACOBIAN_DESTROY, 
				    &chi->omega, x_tmp); 
  ...
  x_update = 0.0;
  for (i=0; i < cgi->numOwnedUnks; i++) delta_x[i] = -(a[i] + dt_p * b[i]);
  for (i=0; i < cgi->numOwnedUnks; i++) 
    x_update += fabs(delta_x[i])/(fabs(x[i])*reltol + abstol)*
      fabs(delta_x[i])/(fabs(x[i])*reltol + abstol);
  ...
  for (i=0; i < cgi->numOwnedUnks; i++) {
    ...
    delta_y = e[i];
    delta_z = f[i]; 
    y_update += fabs(delta_y)/(fabs(chi->y_vec[i])*reltol + abstol)*
      fabs(delta_y)/(fabs(chi->y_vec[i])*reltol + abstol);
    z_update += fabs(delta_z)/(fabs(chi->z_vec[i])*reltol + abstol)*
      fabs(delta_z)/(fabs(chi->z_vec[i])*reltol + abstol); 
    chi->y_vec[i] += delta_y;
    chi->z_vec[i] += delta_z;
  ...
  if ((x_update < 1.0) && (param_update < 1.0) && (omega_update < 1.0) && 
      (y_update < 100.0) && (z_update < 100.0)) {
    first2=TRUE;                      
    return(TRUE);
  }
  else
...

{
 ...
 /*************************** BEGIN EXECUTION *******************************/
  ...
  for (i=0; i < cgi->numUnks; i++) c[i] *= -1.0;
  ...
  eps = cgi->perturb * sqrt( dp(x,x) / (dp(a,a) + cgi->perturb)
                      + dp(x2,x2) / (dp(c,c) + cgi->perturb));
  ...
  eps = cgi->perturb * sqrt(dp(x,x) / (dp(b,b) + cgi->perturb)
                     + dp(x2,x2) / (dp(d,d) + cgi->perturb));
  ...
  for (i=0; i<cgi->numUnks; i++) delta_x[i] = c[i] + dt_p*d[i];
  ...
  param_update = fabs(dt_p) /
               (reltol * fabs(con->phase_transition_info.bif_param) + abstol);

  /* update x2 and modify delta_x for update in usual Newton routine */

  for (i=0; i<cgi->numUnks; i++) x2[i]     += delta_x[i];
  for (i=0; i<cgi->numUnks; i++) delta_x[i] = -a[i] - dt_p*b[i];
...
int manifold_alg(double *x, double *delta_x, struct con_struct *con,
		                        double reltol, double abstol)
{
#ifdef LOCA_MF
 ...
 /* Bordering algorithm for multi-parameter arc-lenght continuation */
  ...
  /* shorthand for long con sub-structure */
  struct general_info_struct  *cgi = &(con->general_info);
  struct manifold_info_struct *cmi = &(con->manifold_info);

  double *a, **b, *rhs;
  int i,j, converged=FALSE;
  double *ux, *u0x, *up, *u0p, param_update=0.0;
  double *kbyk, *rhsp, *col_ix, *col_ip; 
  int *ipiv, info=0, *dim, one=1;
  static char cflag='N';
  ...
  b = (double **) malloc(cmi->k * sizeof(double *));
  for (i=0; i<cmi->k; i++) b[i] = alloc_vec();

  for (i=0; i<cgi->numUnks; i++) a[i] = -delta_x[i]; 

  ux = MFLOCANVX(cmi->u);
  u0x = MFLOCANVX(cmi->u0);
  up = MFLOCANVP(cmi->u);
  u0p = MFLOCANVP(cmi->u0);

  /* Calculate dx/dp  by solving inv(J)*dR/dp for each parameter */

  matrix_residual_fill_conwrap(x, rhs, RHS_ONLY);
  
  for (i=0; i<cmi->k; i++) {

    calc_rhs_multi(up, i, cmi->k, b[i], rhs, x,
                           cgi->perturb, cgi->numUnks);

    /* Solver for dx/dp tangent vector */
    (void) linear_solver_conwrap(b[i], OLD_JACOBIAN, NULL);
  }

  /* Set up k-by-k matric problem for delta_p vector */

  kbyk = (double *) malloc(cmi->k * cmi->k * sizeof(double));
  ipiv = (int *) malloc(cmi->k * sizeof(int));
  rhsp = (double *) malloc(cmi->k * sizeof(double));

  for (i=0; i<cmi->k; i++) {
    col=MFMColumn(cmi->phi, i);
    ...
    for (j=0; j<cmi->k; j++) rhsp[i] -= col_ip[j]*(up[j]-u0p[j]);

    rhsp[i] -= scaled_dp(col_ix, a);

    for (j=0; j<cmi->k; j++) {
       kbyk[i + j*cmi->k] = scaled_dp(col_ix, b[j]) + col_ip[j];
    }
    MFFreeNVector(col);
  }

  dim = &(cmi->k);
  (void *) dgetrf_(dim, dim, kbyk, dim, ipiv, &info);
  if (info==0)
    (void *) dgetrs_(&cflag, dim, &one, kbyk, dim, ipiv, rhsp, dim, &info);

  /* Newton update for parameter vector p, and set -update in delta_x */
  param_update = 0.0;
  for (i=0; i<cmi->k; i++)  {
     param_update += fabs(rhsp[i]) / (reltol * fabs(up[i]) + abstol);
     up[i] += rhsp[i];
     for (j=0; j<cgi->numOwnedUnks; j++)
        delta_x[j] -= rhsp[i]*b[i][j];
  }

  assign_multi_parameter_conwrap(up);

  if (param_update < 1.0) converged=TRUE;
  else converged = FALSE;

  if (cgi->printproc) {

    printf("\n\tManifold Continuation: Convergence Criteria\n");
    printf("\tVariable     Scaled Update (<1)  Unscaled Update  New Value\n");
    printf("\t***********************************************************\n");
    for (i=0; i<cmi->k; i++)
    printf("\tparameter(%d)    %e        %e    %g\n",
	   i, param_update, rhsp[i], up[i]);
    printf("\t***********************************************************\n");
  }
  ...
  free_vec (&a);
  free_vec (&rhs);
  for (i=0; i<cmi->k; i++) free_vec (&(b[i]));
  free(b);
 ...
 */
{
  return perturb * (perturb + fabs(param));
}
/*****************************************************************************/
/*****************************************************************************/
/*****************************************************************************/
...
void calc_rhs_continuation(int rhs_type, double *x, double *resid_vector,
			   ...
			   double param, double perturb, double *r_vec,
...
*/
   ...
   */
    ...
    for (i=0; i< numUnks; i++) r_vec[i] = -resid_delta[i]; 
...
/*****************************************************************************/
#ifdef LOCA_MF
void calc_rhs_multi(double *param_vec, int i, int k,  double *phix,
                    double *rhs, double *x, double perturb, int numUnks)
{
  ...
  param_vec[i] += dc_p;
  assign_multi_parameter_conwrap(param_vec);

  vec_init(phix);
  matrix_residual_fill_conwrap(x, phix, RHS_ONLY);

  param_vec[i] = p_init;;
  assign_multi_parameter_conwrap(param_vec);

  for (j=0; j<numUnks; j++) phix[j] = -(phix[j] - rhs[j])/dc_p;
                      ...
                      double reltol, double abstol)
{
   ...
   */
   ...
   else if (cgi->method == ZERO_ORDER_CONTINUATION ||
            cgi->method == FIRST_ORDER_CONTINUATION ||
            cgi->method == LOCA_LSA_ONLY ||
	    (cgi->method == ARC_LENGTH_CONTINUATION && cpi->step_num == 0) ||
            (cgi->method == MANIFOLD_CONTINUATION && cpi->step_num == 0)) {
     converged = TRUE;
   }

   else if (cgi->method == MANIFOLD_CONTINUATION) {
