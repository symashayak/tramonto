...
static char *cvs_util_id =
  "$Id: loca_util.c,v 1.8 2007/03/09 20:57:47 ljfrink Exp $";
#endif
*/
...
double *SclVec;   /* Pointer to the scaling vector, so that routines that 
                     use it won't have to pass it in */
...
/* simple dot product scaled by scaling vector and length */
{
  ...
  for (i=0; i<N_o; i++) sum += x[i]*y[i]*SclVec[i]*SclVec[i];

  /* sum *= 0.001; */
  return gsum_double_conwrap(sum/N_g);
}
/*****************************************************************************/
/*****************************************************************************/
/*****************************************************************************/
...
double null_vector_resid(double r_val, double i_val,
 ...
 */
      ...
      if (mm_flag) mass_matvec_mult_conwrap(vr, v1);
      else         vec_copy(vr, v1);
 ...
 */
  ...
  for (i=0; i<ntd; i++) td[i] = 0.0;
  tv = (double *) calloc(ntv, sizeof(double));
  for (i=0; i<ntv; i++) tv[i] = 0.0;
  ...
  /* If this is the second of a complex eigenpair, reset the skip_next flag */
    if (skip) skip_next = FALSE;
    else skip_next = ( (d[i+ncv] == 0.0) ? FALSE : TRUE); 
    for (j=0; j<i; j++) {
